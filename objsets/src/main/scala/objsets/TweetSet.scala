...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  // 这一步非常重要，如果先union left和right，runtime会非常长。因为left union right会同时include descendingByRetweet
  def union(that: TweetSet): TweetSet = left union (right union that).incl(elem)

  def mostRetweeted: Tweet = mostAcc(new Tweet("ini_user", "ini_text", 0))
...
object GoogleVsApple {
  ...
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(y => google.exists(x => y.text.contains(x)))
  /**
